...
class GameSession:
    ...
    def get_player_names(self):
        ...
        while getting_names:
            ...
            # راهنما
            title_surf = font.render("🎮 Enter Player Names", True, ACCENT_COLOR)
            screen.blit(title_surf, (SCREEN_WIDTH // 2 - title_surf.get_width() // 2, SCREEN_HEIGHT // 2 - 220))
            
            # باکس بازیکن اول
            p1_label = font.render("First Player Name:", True, TEXT_COLOR)
            screen.blit(p1_label, (SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT // 2 - 140))
            p1_box = pygame.Rect(SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT // 2 - 100, 500, 45)
            ...
            screen.blit(p1_surf, (SCREEN_WIDTH // 2 - 240, SCREEN_HEIGHT // 2 - 90))
            
            # باکس بازیکن دوم
            p2_label = font.render("Second Player Name:", True, TEXT_COLOR)
            screen.blit(p2_label, (SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT // 2 - 20))
            p2_box = pygame.Rect(SCREEN_WIDTH // 2 - 250, SCREEN_HEIGHT // 2 + 20, 500, 45)
            ...
            screen.blit(p2_surf, (SCREEN_WIDTH // 2 - 240, SCREEN_HEIGHT // 2 + 30))
            
            # دکمه تایید
            btn_box = pygame.Rect(SCREEN_WIDTH // 2 - 80, SCREEN_HEIGHT // 2 + 120, 160, 50)
            ...
            screen.blit(btn_surf, (SCREEN_WIDTH // 2 - btn_surf.get_width() // 2, SCREEN_HEIGHT // 2 + 130))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if p1_box.collidepoint(event.pos):
                        active_input = 1
                    elif p2_box.collidepoint(event.pos):
                        active_input = 2
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_RETURN:
                        ...
                        if event.unicode and ord(event.unicode) >= 32:
                            ...
                            elif len(p2_text) < 15 and active_input == 2:
...
class ArcadeMenu:
    def __init__(self, session, screen): # اینجا screen را اضافه کنید
        self.session = session
        self.screen = screen # اینجا ذخیره کنید
